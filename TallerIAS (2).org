...
se ejecuta el lado izquierdo y luego el derecho. El contador de programa inicia
en la posición 300. El set de instrucciones del computador o ISA[fn:2] está
definido en la Tabla \ref{tab:tablaISA-IAS}. A fin de terminar el programa se
agrega la instrucción ~0x99~ que lleva al computador al estado de HALT. Conteste
las siguientes preguntas considerando la Tabla [[tab:memoria-compu]] como el
...
Al finalizar la ejecución del programa, el acumulador contiene el valor:

=AC = 7=

2. *¿Se sobrescribe algún registro como resultado de la ejecución? Si verdadero,
indique qué registro y con qué valor.*

Sí. Durante la ejecución del programa se sobrescriben las siguientes posiciones
de memoria:

- La posición =M(940)= cambia de 5 a 7.
- La posición =M(941)= cambia de 2 a 7.

3. *¿Cuál es el valor del bit del Carry? ¿Qué operación modifica el bit?*

El valor final del bit Carry es:

=Carry = 0=

El bit Carry puede ser modificado por las instrucciones aritméticas =ADD= y
=SUB=. En este programa ninguna operación produce desbordamiento, por lo que el
bit Carry no se activa.
...
#+begin_src asm
.org 0x300
LOAD M(940)      ; lado izquierdo
ADD  M(941)      ; lado derecho

.org 0x301
STOR M(940)      ; lado izquierdo
SUB  M(000)      ; lado derecho

.org 0x302
STOR M(941)      ; lado izquierdo
HALT             ; lado derecho
...
*** Código RTL
Escriba en notación de transferencia de registros el programa que ejecuta el
computador IAS.
